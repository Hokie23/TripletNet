...
function ResidualDrop:__init(deathRate, nChannels, nOutChannels, stride)
    ...
    elseif nOutChannels < nChannels then
       ...
       self.skip:add( cudnn.SpatialConvolution(nChannels, nOutChannels, 1, 1, 1, 1) )
    end
...
function ResidualDrop:updateOutput(input)
    ...
    if self.train then
      if torch.rand(1) < self.deathRate then -- only compute convolutional output when gate is open
        self.gate = true
