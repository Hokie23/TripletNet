...
function PairwiseDistanceOffset:updateOutput(inputset)
    ...
    local input = {inputset[1][1], inputset[2][1]}
    local offset = {inputset[1][2], inputset[2][2]}
    if input[1]:dim() == 1 then
        self.output:resize(1)
        self.output[1]=input[1]:dist(input[2],self.norm) - offset[1]
    elseif input[1]:dim() == 2 then
        ...
        self.output:pow(1./self.norm):add( -1, offset[1] )
    else
...
function PairwiseDistanceOffset:updateGradInput(inputset, gradOutput)
    ...
    self.gradInput[1] = {(self.gradInput[1] or input[1].new()):resize(input[1]:size()) , offset*self.grandOutput[1]}
    self.gradInput[2] = {(self.gradInput[2] or input[2].new()):resize(input[2]:size()), 0}
    self.gradInput[1][1]:copy(input[1])
    self.gradInput[1][1]:add(-1, input[2]) 

    if self.norm==1 then
        self.gradInput[1][1]:apply(mathsign)
    else
        -- Note: derivative of p-norm:
        -- d/dx_k(||x||_p) = (x_k * abs(x_k)^(p-2)) / (||x||_p)^(p-1)
        if (self.norm > 2) then
            self.gradInput[1][1]:cmul(self.gradInput[1]:clone():abs():pow(self.norm-2))
        end
        ...
        if (input[1]:dim() > 1) then
            ...
            self.gradInput[1][1]:cmul(self.outExpand:expand(self.gradInput[1]:size(1),
            self.gradInput[1][1]:size(2)))
        else
            self.gradInput[1][1]:mul(math.pow(self.output[1] + 1e-6, -(self.norm-1)))
        end
    end
    if input[1]:dim() == 1 then
        self.gradInput[1][1]:mul(gradOutput[1])
    else
        ...
        self.gradInput[1][1]:cmul(self.grad)
    end
    self.gradInput[2][1]:zero():add(-1, self.gradInput[1][1])
